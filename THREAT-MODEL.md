# KoIME 脅威モデル — 何から守り、何は守らないか

**日本語** | [English](THREAT-MODEL.en.md)

KoIME の API キーは端末内に**平文で**保存される（詳細は
...
KoIME には運営者・代理人が存在しない。守るべき相手は「あなたの端末に
...
| IME による入力内容の吸い上げ（このアプリ自身への疑い） | 収集コードが存在しないことを**ソースで確認できる**。読むべきは `KoimeImeService.kt`（commitText 経路）と `OpenAiSttEngine.kt`（唯一の通信先） |
