# API キーはどこに・どう保存されるか — KoIME のストレージセキュリティ

KoIME は BYOK（ユーザー自身の API キーを持ち込む）設計のため、
「キーは端末のどこに・どんな形式で置かれ、何から守られているのか」を
利用者・フォーク者が自分で検証できるように、ここに全部書いておく。

記述は 2026-07 時点の Android の仕組みに基づく。コードの現物は
`app/src/main/java/dev/sakai/KoIME/settings/Prefs.kt` と
...
- **場所**: `/data/data/dev.sakai.koime/shared_prefs/koime_prefs.xml`
...
adb シェルは `shell` ユーザーで動き、`/data/data/dev.sakai.koime` は
...
`adb shell run-as dev.sakai.koime` でアプリ本人になりすまして
...
そのため KoIME は `android:dataExtractionRules` を追加し、クラウドバックアップと
