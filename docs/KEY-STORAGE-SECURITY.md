...
`app/src/main/AndroidManifest.xml` を参照（この文書より現物が正）。

## TL;DR

- キーと設定は**アプリ専用領域に平文 XML** で保存される。アプリ自身は暗号化しない。
- 守っているのは Android の多層防御：アプリサンドボックス（他アプリ・USB コピーから）、
  `allowBackup="false"`（バックアップ経由の流出封じ）、ファイルベース暗号化 FBE（端末盗難から）。
...
### ④ バックアップ経由の流出 → 設計で塞いである

マニフェストに `android:allowBackup="false"` を設定済み。
`adb backup` も Google の自動バックアップ（クラウドへの prefs 吸い上げ）も
対象外になり、**キーが端末の外に出る経路が存在しない**。

トレードオフとして機種変更時にキーは引き継がれない。BYOK の利用者は
キーを自分の管理体制（パスワードマネージャ等）で持っている前提なので、
再入力（Autofill フィル、DD-012）で足りると判断している。

### ⑤ 端末ごと盗まれた・分解された → FBE が守る

Android 10 以降必須のファイルベース暗号化（FBE）で `/data` 全体が暗号化され、
鍵はロック画面の資格情報と TEE（セキュアチップ）に紐づく。
PIN を知らなければ、フラッシュメモリを直接読み出しても復号できない。
...
「アプリ専用領域に平文＋サンドボックス＋FBE＋backup 無効」は、
世の中の大半のアプリがログイントークンやセッション Cookie を
保存しているのと同じ水準。特別に弱くも強くもない、業界の標準線。

## 4. さらに固める選択肢と、いま採らない理由

Android Keystore（TEE）で AES 鍵を作り、キーを暗号化してから保存する方式がある。
冷静に評価すると、追加で防げるのは「FBE を突破されて `/data` の平文は取られたが
...
- iOS の `UserDefaults`（SharedPreferences 相当）は平文 plist だが、
  **Finder / iCloud バックアップにデフォルトで含まれる**。Android で
  `allowBackup="false"` に相当する遮断を明示しない限り、
