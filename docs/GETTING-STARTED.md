...
KoIME はストア配布もバイナリ配布もしない。**自分でビルドして入れる**のが
...
2. このとき OS が「このキーボードは入力内容をすべて収集できる可能性がある」
   ...
   標準警告**である。KoIME が何を送り何を送らないかはソースで確認できる
   （[THREAT-MODEL.md](../THREAT-MODEL.md) の該当行参照）。

## 3. マイク権限

ランチャーから **KoIME** の設定画面を開き、「マイクを許可」から
...
1. 任意のアプリの入力欄でキーボードを KoIME に切り替える
   （地球儀キー、または OS のキーボード切替）。
2. **マイクをタップ → 話す → もう一度タップ**。数秒後、転写テキストが
   カーソル位置に挿入される。録音上限は 5 分。
3. 補助キー:
   - **◀▶**: カーソル移動（長押しでリピート）
   - **UNDO**: 直前の転写だけを一括で取り消す。
     カーソル移動や手編集の後は、安全のため効かなくなる
     （無関係のテキストを消さないための一致検証つき）
   - SPACE / ⌫ / ⏎ / ⚙（設定）

   UI の表記は意図的に英語のみ（中学英語レベル）。理由と全文字列の対訳は
   [LOCALIZATION.md](LOCALIZATION.md) を参照。
...
- 失敗は必ずキーボードの状態表示に理由が出る設計（無言で戻ることはない）。
  さらに追うなら: `adb logcat -s koime AndroidRuntime`
