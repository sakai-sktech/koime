...
この文書は、KoIME を自分でビルドして署名する人（GETTING-STARTED §6）と、
...
## KoIME にとってこれが何を意味するか

KoIME はストア配布もバイナリ配布もしない。**自分でビルドして自分の鍵で署名する**のが正規の導入経路である（GETTING-STARTED）。上の地図に置くと:

- **連続性（LAYER 1）はあなたの鍵が保証する。**
  `keytool` で作った自己署名鍵は「オレオレ」だが、それは Android の正規設計そのものであり、手抜きではない。
  あなたの端末上の KoIME が、あなたのビルドからしか更新されないことを保証する。
